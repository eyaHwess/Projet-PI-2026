        ...
        if (!mediaRecorder || mediaRecorder.state === 'inactive') {
            ...
            try {
                ...
                mediaRecorder.onstop = () => {
                    ...
                    // Show audio preview with send button
                    const preview = document.createElement('div');
                    ...
                    preview.innerHTML = `
                        <button type="button" onclick="document.getElementById('voiceAudio').play()" style="width: 32px; height: 32px; border-radius: 50%; background: #0084ff; border: none; color: white; cursor: pointer;">
                            <i class="fas fa-play"></i>
                        </button>
                        <audio id="voiceAudio" src="${audioUrl}" style="display: none;"></audio>
                        <div style="flex: 1;">
                            <div style="font-size: 13px; font-weight: 600;">Voice message</div>
                            <div style="font-size: 12px; color: #65676b;">${recordingSeconds}s</div>
                        </div>
                        <button type="button" id="sendVoiceBtn" style="width: 36px; height: 36px; border-radius: 50%; background: #10b981; border: none; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                            <i class="fas fa-paper-plane"></i>
                        </button>
                        <button type="button" onclick="document.getElementById('voicePreview').remove(); currentAudioBlob = null;" style="background: none; border: none; color: #65676b; cursor: pointer;">
                            <i class="fas fa-times"></i>
                        </button>
                    `;
                    ...
                    // Add send voice message handler
...
async function sendVoiceMessage() {
    if (!currentAudioBlob) {
        alert('No voice message to send');
        return;
    }
    ...
    // Get the goal ID from window variable or URL
    const goalId = window.GOAL_ID || window.location.pathname.split('/').pop();
    ...
    try {
        ...
        if (data.success) {
            ...
            alert(data.error || 'Error sending voice message');
            if (sendBtn) {
...
if (chatForm) {
    chatForm.addEventListener('submit', function(e) {
        ...
        // Check if there's either content OR an attachment
